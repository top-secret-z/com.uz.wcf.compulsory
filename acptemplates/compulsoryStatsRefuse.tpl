{assign var='pageTitle' value='wcf.acp.compulsory.stats.refuse'}
...
{hascontent}
	<div class="paginationTop">
		{content}
			{pages print=true assign=pagesLinks controller="CompulsoryStatsRefuse" id=$compulsory->compulsoryID link="pageNo=%d&sortField=$sortField&sortOrder=$sortOrder"}
		{/content}
	</div>
{/hascontent}
		...
		<table class="table">
			<thead>
				<tr>
					...
					<th class="columnTitle columnUsername{if $sortField == 'username'} active {@$sortOrder}{/if}" colspan="2"><a href="{link controller='CompulsoryStatsRefuse' id=$compulsory->compulsoryID}pageNo={@$pageNo}&sortField=username&sortOrder={if $sortField == 'username' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{/link}">{lang}wcf.user.username{/lang}</a></th>
					
					{if $__wcf->session->getPermission('admin.user.canEditMailAddress')}
						<th class="columnText columnEmail{if $sortField == 'email'} active {@$sortOrder}{/if}"><a href="{link controller='CompulsoryStatsRefuse' id=$compulsory->compulsoryID}pageNo={@$pageNo}&sortField=email&sortOrder={if $sortField == 'email' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{/link}">{lang}wcf.user.email{/lang}</a></th>
					{/if}
					<th class="columnDigits columnActivityPoints{if $sortField == 'activityPoints'} active {@$sortOrder}{/if}"><a href="{link controller='CompulsoryStatsRefuse' id=$compulsory->compulsoryID}pageNo={@$pageNo}&sortField=activityPoints&sortOrder={if $sortField == 'activityPoints' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{/link}">{lang}wcf.user.sortField.activityPoints{/lang}</a></th>
					{if MODULE_LIKE}
						<th class="columnDigits columnLikesReceived{if $sortField == 'likesReceived'} active {@$sortOrder}{/if}"><a href="{link controller='CompulsoryStatsRefuse' id=$compulsory->compulsoryID}pageNo={@$pageNo}&sortField=likesReceived&sortOrder={if $sortField == 'likesReceived' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{/link}">{lang}wcf.like.cumulativeLikes{/lang}</a></th>
					{/if}
					{if MODULE_TROPHY}
						<th class="columnDigits columnTrophyPoints{if $sortField == 'trophyPoints'} active {@$sortOrder}{/if}"><a href="{link controller='CompulsoryStatsRefuse' id=$compulsory->compulsoryID}pageNo={@$pageNo}&sortField=trophyPoints&sortOrder={if $sortField == 'trophyPoints' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{/link}">{lang}wcf.user.trophy.trophyPoints{/lang}</a></th>
					{/if}
					<th class="columnDate columnRegistrationDate{if $sortField == 'registrationDate'} active {@$sortOrder}{/if}"><a href="{link controller='CompulsoryStatsRefuse' id=$compulsory->compulsoryID}pageNo={@$pageNo}&sortField=registrationDate&sortOrder={if $sortField == 'registrationDate' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{/link}">{lang}wcf.user.registrationDate{/lang}</a></th>
					<th class="columnDate columnLastActivityTime{if $sortField == 'lastActivityTime'} active {@$sortOrder}{/if}"><a href="{link controller='CompulsoryStatsRefuse' id=$compulsory->compulsoryID}pageNo={@$pageNo}&sortField=lastActivityTime&sortOrder={if $sortField == 'lastActivityTime' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{/link}">{lang}wcf.user.lastActivityTime{/lang}</a></th>
				</tr>
			</thead>
